...
CHANGE 1: Symlog transform before standardisation
============================================================
CFD fields (Cp, WSS) have heavy tails — values reaching ±27σ or ±50σ
after standard scaling. This causes MSE loss to be dominated by
a few extreme vertices, driving the model to predict near-zero.

Fix: apply a symmetric log transform BEFORE standardising.
    symlog(y) = sign(y) * log(1 + |y|)

This compresses tails while preserving sign and zero-crossing.
After symlog, standard scaling works well because the distribution
is much closer to Gaussian.

At eval time, invert with:
    inv_symlog(y') = sign(y') * (exp(|y'|) - 1)
"""
...
# ────────────────────────────────────────────────────────────────────────────
# Field normalisation — NOW WITH SYMLOG
# ────────────────────────────────────────────────────────────────────────────

def normalise_fields(cp: Tensor, wss: Tensor):
    """
    Normalise Cp and WSS fields for training:
      1. Apply symlog to compress heavy tails
      2. Standardise to zero mean, unit variance

    Returns (cp_norm, wss_norm, stats) where stats stores everything
    needed to invert at eval time.
    """
    # ── Step 1: symlog compress ──────────────────────────────────
    cp_sl  = symlog(cp)
    wss_sl = symlog(wss)

    # ── Step 2: standardise the symlog'd values ─────────────────
    cp_mean = cp_sl.mean()
    cp_std  = cp_sl.std().clamp(min=1e-8)
    cp_norm = (cp_sl - cp_mean) / cp_std

    wss_mean = wss_sl.mean()
    wss_std  = wss_sl.std().clamp(min=1e-8)
    wss_norm = (wss_sl - wss_mean) / wss_std

    stats = {
        'cp_mean': cp_mean,  'cp_std': cp_std,
        'wss_mean': wss_mean, 'wss_std': wss_std,
        'used_symlog': True,  # flag so denorm knows to invert
    }
    return cp_norm, wss_norm, stats


def denormalise_cp(cp_norm: Tensor, stats: dict) -> Tensor:
    """Invert normalisation: unstandardise, then inv_symlog."""
    cp_sl = cp_norm * stats['cp_std'] + stats['cp_mean']
    if stats.get('used_symlog', False):
        return inv_symlog(cp_sl)
    return cp_sl


def denormalise_wss(wss_norm: Tensor, stats: dict) -> Tensor:
    """Invert normalisation: unstandardise, then inv_symlog."""
    wss_sl = wss_norm * stats['wss_std'] + stats['wss_mean']
    if stats.get('used_symlog', False):
        return inv_symlog(wss_sl)
    return wss_sl
...
def random_rotation_augment(x: Tensor, angles: Tensor, transporters: Tensor,
    ...
    """
    import math
    theta = torch.randn(1).item() * math.radians(sigma_deg)
    c, s = math.cos(theta), math.sin(theta)

    xyz = x[:, :3].clone()
    x_rot = xyz[:, 0] * c - xyz[:, 1] * s
    y_rot = xyz[:, 0] * s + xyz[:, 1] * c

    x_new = x.clone()
    x_new[:, 0] = x_rot
    x_new[:, 1] = y_rot
    return x_new, angles, transporters
